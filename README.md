...
- **Fit-верификатор: массовый unknown ≠ skip** — клауза `unkN >= 3` в
  `finish()` удалена: 5 закрытых must-have + 3 unknown теперь дают
  `caveats`, а не ложный `skip`. Проверяющий цикл (`TestMassUnknown`,
  `TestMapCoverageMissingItemFallback`) страхует регресс.
- **Fallback на coverage() при пропуске моделью** — если LLM-движок
  не разметил требование (len(cov.Items) != len(prompted)),
  `MapCoverage` вызывает `coverage()` напрямую: требование попадает в
  `unknown`/`missing` по факту, а не молча исчезает.
- **Концепт «реляционные БД и SQL»** — новая запись в `DefaultConcepts()`
  с триггерами `реляционн|база данных|бд|sql` и сигналами
  `pg_stat_statements`, `EXPLAIN`, `индексы`; закрывает must-have
...
- **Гибридный движок включён по умолчанию** — `settings.json`
  ...
  цитатами + код-верификацию; детерминированный движок остаётся
  fallback.
- **Anti-fabrication** — таблица ограничений и глобальные запреты в системном промпте запрещают приписывать технологии из вакансии к буллетам письма. Честный пробел («не работал, готов освоить») остаётся.
